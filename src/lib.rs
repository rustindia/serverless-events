...
fn handle_serverless_events(_req: Request) -> Result<Response> {
    ...
    if diff > one_hour {
        ...
        let rust_mumbai_data: DataRustIndiaEvents = parse_luma_fragment(body)?;
...
fn parse_meetup_fragment(body: String, loc_url: &str, array_events: &mut Vec<DataRustIndiaEvents>) {
...
fn parse_hasgeek_past_event_fragment(
    past_body: String,
    loc_url: &str,
    array_events: &mut Vec<DataRustIndiaEvents>,
) {
...
fn parse_luma_fragment(body: String) -> Result<DataRustIndiaEvents> {
    let document = Html::parse_document(&body);
